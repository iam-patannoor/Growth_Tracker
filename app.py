...
st.markdown("""
    ...
    h3 { color: #3A506B; }
    .stButton>button { background-color: #2B3E50; color: white; border-radius: 8px; }
...
st.markdown("<p style='text-align: center; italic: true; color: #555;'>\"Indeed, Allah will not change the condition of a people until they change what is in themselves.\" (Quran 13:11)</p>", unsafe_allow_html=True)
st.markdown("---")
...
# Layout Pillars using Columns
tab1, tab2, tab3 = st.tabs(["🕌 Spiritual Core", "💻 Growth & Independence", "🛡️ Mind Detox Shield"])
...
with tab1:
    ...
    islamic_time = st.number_input("📚 Islamic Class / Reading (Minutes)", min_value=0, step=5)

with tab2:
    st.markdown("### Build Your Future & Independence")
    tech_time = st.number_input("🚀 Technical Upskilling & Study (Minutes)", min_value=0, step=5)
    workout_time = st.number_input("🏃‍♂️ Physical Workout / Brisk Walk (Minutes)", min_value=0, step=5)
...
if st.button("💾 Save My Daily Progress"):
    st.balloons()
    st.markdown("<div class='success-box'>Entry Saved Professionally!</div>", unsafe_allow_html=True)
    
    # Custom Feedback based on user performance
    if total_score >= 7:
